...
@Component({
  ...
  template: `
    ...
    -->
    <input matInput #pippo="ngModel" type="text" [attr.name]="item.id" placeholder="{{placeholder()}}" [(ngModel)]="item.value"
           pattern="{{pattern}}" (change)="onChange($event)" [required]="item.mandatory">
    <!--
