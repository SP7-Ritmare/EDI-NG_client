<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
    ...
    <xsl:template match="item">
        <item>
            ...
            <xsl:if test="@min">
                <xsl:element name="min"><xsl:value-of select="@min" /></xsl:element>
            </xsl:if>
            <xsl:if test="@max">
                <xsl:element name="max"><xsl:value-of select="@max" /></xsl:element>
            </xsl:if>
            <xsl:if test="@step">
                <xsl:element name="step"><xsl:value-of select="@step" /></xsl:element>
            </xsl:if>
