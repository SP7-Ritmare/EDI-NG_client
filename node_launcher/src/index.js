...
*/
  ...
  const { error1, stdout1, stderr1 } = await execFile("node_modules/.bin/bower", ["update"], {cwd: ".."});
  if (error1) {
    logger.error("Error running bower update", error)
    process.exit(1)
  }

  const { error2, stdout2, stderr2 } = await execFile("node_modules/.bin/grunt", [], {cwd: ".."});
  if (error2) {
    logger.error("Error running grunt", error)
    process.exit(1)
  }
